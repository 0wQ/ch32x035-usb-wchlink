...
static bool rvswd_flash_ch5xx_flash_enable_code_mode(
    ...
    if (!rvswd_debug_write_raw_gpr(operation, 13u, RVSWD_CH5XX_FLASH_KEY_ADDRESS) ||
...
static bool rvswd_flash_ch5xx_flash_wait_ready(struct rvswd_ch5xx_byte_access *access) {
    if (!rvswd_flash_ch5xx_flash_end(access)) {
        access->operation->flash_code = RVSWD_FLASH_ERROR_CH5XX_COMMAND_FINISH;
        return false;
    }
    ...
    for (uint32_t retry = 0u; retry < RVSWD_CH5XX_FLASH_STATUS_RETRIES; ++retry) {
        ...
        if (!rvswd_flash_ch5xx_flash_begin(access, 5u)) {
            access->operation->flash_code = RVSWD_FLASH_ERROR_CH5XX_STATUS_BEGIN;
            return false;
        }
        if (!rvswd_flash_ch5xx_flash_in(access, &status)) {
            access->operation->flash_code =
                RVSWD_FLASH_ERROR_CH5XX_STATUS_READ_FIRST;
            return false;
        }
        if (!rvswd_flash_ch5xx_flash_in(access, &status)) {
            access->operation->flash_code =
                RVSWD_FLASH_ERROR_CH5XX_STATUS_READ_SECOND;
            return false;
        }
        if (!rvswd_flash_ch5xx_flash_end(access)) {
            access->operation->flash_code = RVSWD_FLASH_ERROR_CH5XX_STATUS_FINISH;
            return false;
        }
        if ((status & 1u) == 0u) {
            return true;
    ...
    access->operation->flash_code = RVSWD_FLASH_ERROR_CH5XX_STATUS_TIMEOUT;
    return false;
}
...
static bool rvswd_flash_ch5xx_flash_erase_command(
    ...
    if (!rvswd_flash_ch5xx_flash_begin(access, command)) {
        access->operation->flash_code = RVSWD_FLASH_ERROR_CH5XX_COMMAND_BEGIN;
        return false;
    }
    if (!rvswd_flash_ch5xx_flash_write_address(access, address)) {
        access->operation->flash_code = RVSWD_FLASH_ERROR_CH5XX_COMMAND_ADDRESS;
        return false;
    }
...
static bool rvswd_flash_ch5xx_flash_program_page(
    ...
    for (uint32_t offset = 0u; offset < RVSWD_CH5XX_FLASH_PAGE_SIZE; offset += 4u) {
        ...
        if (!rvswd_debug_wait_abstract_idle_timeout(
            ...
            access->operation->flash_code =
                RVSWD_FLASH_ERROR_CH5XX_PAGE_PROGRAM_WAIT;
            return false;
        }
        if (((abstractcs >> 8u) & 0x07u) != 0u) {
            access->operation->flash_code =
                RVSWD_FLASH_ERROR_CH5XX_PAGE_PROGRAM_STATUS;
            return false;
        }
    }
    return true;
...
bool rvswd_flash_rewrite_page(struct rvswd_operation *operation,
    ...
    if ((address & 0xffu) != 0u) {
        operation->flash_code = RVSWD_FLASH_ERROR_CH5XX_PAGE_UNALIGNED;
        return false;
    }

    // 官方 0x0A 路径先用命令 0x81 擦页，再独立建立命令 0x02 的整页写入状态
    if (!rvswd_flash_ch5xx_flash_enable_code_mode(operation)) {
        operation->flash_code = RVSWD_FLASH_ERROR_CH5XX_PAGE_ERASE_CODE_MODE;
        return false;
    }
    if (!rvswd_flash_ch5xx_flash_open(&access)) {
        operation->flash_code = RVSWD_FLASH_ERROR_CH5XX_PAGE_ERASE_OPEN;
        return false;
    }
    ...
    if (!rvswd_flash_ch5xx_flash_enable_code_mode(operation)) {
        operation->flash_code = RVSWD_FLASH_ERROR_CH5XX_PAGE_PROGRAM_CODE_MODE;
        return false;
    }
    if (!rvswd_flash_ch5xx_flash_open(&access)) {
        operation->flash_code = RVSWD_FLASH_ERROR_CH5XX_PAGE_PROGRAM_OPEN;
        return false;
    }
    if (!rvswd_flash_ch5xx_flash_begin(&access, 0x02u) ||
        !rvswd_flash_ch5xx_flash_write_address(&access, address)) {
        operation->flash_code = RVSWD_FLASH_ERROR_CH5XX_PAGE_PROGRAM_BEGIN;
        return false;
    }
...
static bool rvswd_flash_ch5xx_flash_erase_all(
    ...
    if (stub_length == 0u || stub_length > RVSWD_CH5XX_ERASE_STUB_MAX_SIZE ||
        (stub_length & 3u) != 0u) {
        operation->flash_code = 0xc1u;
        return false;
    }
    if (!rvswd_memory_write(operation, profile, RVSWD_CH5XX_ERASE_STUB_ADDRESS,
                            ch5xx_flash_erase_stub_start,
                            (uint32_t)stub_length)) {
        operation->flash_code = 0xc2u;
        return false;
    }
    if (!rvswd_flash_ch5xx_flash_enable_code_mode(operation)) {
        operation->flash_code = 0xc3u;
        return false;
    }
    if (!rvswd_debug_execute(operation, RVSWD_CH5XX_ERASE_STUB_ADDRESS,
        ...
        operation->flash_code = 0xc4u;
        return false;
    }
    if (result != 0u) {
        operation->flash_code = 0xc5u;
        return false;
    }
    return true;
}
...
bool rvswd_flash_erase_all(struct rvswd_operation *operation,
    ...
    if (profile->ch5xx_protocol) {
        return rvswd_flash_ch5xx_flash_erase_all(operation, profile);
    }
