...
static bool rvswd_memory_write16(struct rvswd_transport *transport,
...
static bool rvswd_flash_wait_ready(struct rvswd_transport *transport,
    ...
    do {
        if (!rvswd_memory_read32(transport, profile, true, RVSWD_FLASH_STATR_ADDRESS,
                                 status)) {
...
static bool rvswd_flash_unlock_main_option_and_fast(
    struct rvswd_transport *transport, uint32_t control) {
    if ((control & (RVSWD_FLASH_CTLR_LOCK | RVSWD_FLASH_CTLR_FAST_LOCK)) == 0u) {
        return true;
    }

    // L103 需要同时解锁主存储区、用户字和快速编程模式
    return rvswd_memory_write32(transport, RVSWD_FLASH_KEYR_ADDRESS, RVSWD_FLASH_KEY1) &&
           rvswd_memory_write32(transport, RVSWD_FLASH_KEYR_ADDRESS, RVSWD_FLASH_KEY2) &&
           rvswd_memory_write32(transport, RVSWD_FLASH_OBKEYR_ADDRESS, RVSWD_FLASH_KEY1) &&
           rvswd_memory_write32(transport, RVSWD_FLASH_OBKEYR_ADDRESS, RVSWD_FLASH_KEY2) &&
           rvswd_memory_write32(transport, RVSWD_FLASH_MODEKEYR_ADDRESS, RVSWD_FLASH_KEY1) &&
           rvswd_memory_write32(transport, RVSWD_FLASH_MODEKEYR_ADDRESS, RVSWD_FLASH_KEY2);
}

static bool rvswd_flash_unlock_main_and_fast(struct rvswd_transport *transport,
                                             uint32_t control) {
    if ((control & RVSWD_FLASH_CTLR_LOCK) != 0u &&
        (!rvswd_memory_write32(transport, RVSWD_FLASH_KEYR_ADDRESS, RVSWD_FLASH_KEY1) ||
         !rvswd_memory_write32(transport, RVSWD_FLASH_KEYR_ADDRESS, RVSWD_FLASH_KEY2))) {
        return false;
    }
...
static bool rvswd_flash_ch5xx_flash_enable_code_mode(
    struct rvswd_transport *transport) {
    uint32_t abstractcs;

    // SAFE_ACCESS_SIG 必须在修改 ROM 配置后清零，避免命令口继承未完成的安全访问状态
    if (!rvswd_debug_write_raw_gpr(transport, 13u, RVSWD_CH5XX_FLASH_KEY_ADDRESS) ||
...
bool rvswd_flash_rewrite_page(struct rvswd_transport *transport,
                              const struct rvswd_target_profile *profile,
    ...
    if (!rvswd_flash_ch5xx_flash_enable_code_mode(transport)) {
...
static bool rvswd_flash_ch5xx_flash_erase_all(
    ...
    if (!rvswd_memory_write(transport, profile, RVSWD_CH5XX_ERASE_STUB_ADDRESS,
    ...
    if (!rvswd_debug_execute(transport, RVSWD_CH5XX_ERASE_STUB_ADDRESS,
...
bool rvswd_flash_erase_all(struct rvswd_transport *transport,
                           const struct rvswd_target_profile *profile) {
    uint32_t control;
    ...
    if (profile->ch5xx_protocol) {
        return rvswd_flash_ch5xx_flash_erase_all(transport, profile);
    }

    if (!rvswd_flash_wait_ready(transport, profile, &status, 0x11u, 0x12u)) {
        return false;
    }

    if (!rvswd_memory_read32(transport, profile, true, RVSWD_FLASH_CTLR_ADDRESS,
                             &control)) {
    ...
    switch (profile->erase_unlock) {
        case RVSWD_FLASH_UNLOCK_MAIN_OPTION_AND_FAST:
            unlocked =
                rvswd_flash_unlock_main_option_and_fast(transport, control);
            break;
        case RVSWD_FLASH_UNLOCK_MAIN_AND_FAST:
            unlocked = rvswd_flash_unlock_main_and_fast(transport, control);
            break;
        default:
            rvswd_flash_last_error_value = 0x0fu;
            return false;
    ...
    if ((status & (RVSWD_FLASH_STATR_EOP | RVSWD_FLASH_STATR_WRPRTERR)) != 0u &&
        !rvswd_memory_write32(transport,
    ...
    if (!rvswd_memory_write32(transport, RVSWD_FLASH_CTLR_ADDRESS, idle_control)) {
        rvswd_flash_last_error_value = 0x18u;
        goto cleanup;
    }
    if (!rvswd_memory_write32(transport, RVSWD_FLASH_CTLR_ADDRESS,
                              idle_control | RVSWD_FLASH_CTLR_MER)) {
        rvswd_flash_last_error_value = 0x19u;
        goto cleanup;
    }
    if (!rvswd_memory_write32(transport,
                              RVSWD_FLASH_CTLR_ADDRESS,
                              idle_control | RVSWD_FLASH_CTLR_MER | RVSWD_FLASH_CTLR_STRT)) {
        rvswd_flash_last_error_value = 0x1au;
        goto cleanup;
    }
    if (!rvswd_flash_wait_ready(transport, profile, &status, 0x1bu, 0x1cu)) {
        goto cleanup;
    }
...
bool rvswd_flash_read_protected(struct rvswd_transport *transport,
                                const struct rvswd_target_profile *profile,
                                bool *protected) {
    ...
    if (!rvswd_memory_read32(transport, profile, true, RVSWD_FLASH_OBR_ADDRESS,
                             &option_status)) {
...
bool rvswd_flash_write_protected(struct rvswd_transport *transport,
                                 const struct rvswd_target_profile *profile,
                                 bool *protected) {
    ...
    if (!rvswd_memory_read32(transport, profile, true, RVSWD_FLASH_WPR_ADDRESS,
                             &write_protection)) {
...
static bool rvswd_flash_unlock_option_bytes(struct rvswd_transport *transport,
                                            bool unlock_fast_mode) {
    if (!rvswd_memory_write32(transport, RVSWD_FLASH_KEYR_ADDRESS, RVSWD_FLASH_KEY1)) {
        rvswd_flash_last_error_value = 0xa1u;
        return false;
    }
    if (!rvswd_memory_write32(transport, RVSWD_FLASH_KEYR_ADDRESS, RVSWD_FLASH_KEY2)) {
        rvswd_flash_last_error_value = 0xa2u;
        return false;
    }
    if (unlock_fast_mode) {
        if (!rvswd_memory_write32(transport, RVSWD_FLASH_MODEKEYR_ADDRESS,
                                  RVSWD_FLASH_KEY1)) {
    ...
    if (!rvswd_memory_write32(transport, RVSWD_FLASH_OBKEYR_ADDRESS,
                              RVSWD_FLASH_KEY1)) {
...
static bool rvswd_flash_write_option_bytes_fast_buffer(
    ...
    if (!rvswd_flash_wait_ready(transport, profile, &status, 0x31u, 0x32u) ||
        !rvswd_memory_read32(transport, profile, true, RVSWD_FLASH_CTLR_ADDRESS,
                             &control)) {
        if (rvswd_flash_last_error_value == 0u) {
    ...
    if (!rvswd_flash_unlock_option_bytes(transport, true)) {
        if (rvswd_flash_last_error_value == 0u) {
    ...
    if (!rvswd_memory_write32(transport, RVSWD_FLASH_CTLR_ADDRESS,
        ...
        !rvswd_flash_wait_ready(transport, profile, &status, 0x36u, 0x37u)) {
        if (rvswd_flash_last_error_value == 0u) {
    ...
    if (!rvswd_memory_read32(transport, profile, true, RVSWD_FLASH_CTLR_ADDRESS,
                             &control) ||
        !rvswd_flash_unlock_main_and_fast(transport, control) ||
        !rvswd_memory_read32(transport, profile, true, RVSWD_FLASH_CTLR_ADDRESS,
                             &control)) {
    ...
    if (!rvswd_memory_write32(transport, RVSWD_FLASH_CTLR_ADDRESS,
        ...
        !rvswd_memory_write32(transport,
                              ...
                              idle_control | RVSWD_FLASH_CTLR_FAST_PROGRAM |
                                  RVSWD_FLASH_CTLR_BUFFER_RESET) ||
        !rvswd_flash_wait_ready(transport, profile, &status, 0x3au, 0x3bu) ||
        !rvswd_memory_write32(transport, RVSWD_FLASH_CTLR_ADDRESS, idle_control)) {
        if (rvswd_flash_last_error_value == 0u) {
    ...
    for (uint32_t index = 0u; index < RVSWD_OPTION_BYTES_WORD_COUNT; ++index) {
        if (!rvswd_memory_write32(transport, RVSWD_FLASH_CTLR_ADDRESS,
                                  idle_control | RVSWD_FLASH_CTLR_FAST_PROGRAM) ||
            !rvswd_memory_write32(transport, profile->option_base + index * 4u,
                                  option_words[index]) ||
            !rvswd_memory_write32(transport,
                                  RVSWD_FLASH_CTLR_ADDRESS,
                                  idle_control | RVSWD_FLASH_CTLR_FAST_PROGRAM |
                                      RVSWD_FLASH_CTLR_BUFFER_LOAD) ||
            !rvswd_flash_wait_ready(transport, profile, &status, 0x3du,
                                    0x3eu) ||
            !rvswd_memory_write32(transport, RVSWD_FLASH_CTLR_ADDRESS, idle_control)) {
            if (rvswd_flash_last_error_value == 0u) {
    ...
    if (!rvswd_memory_write32(transport, RVSWD_FLASH_CTLR_ADDRESS,
                              idle_control | RVSWD_FLASH_CTLR_FAST_PROGRAM) ||
        !rvswd_memory_write32(transport, RVSWD_FLASH_ADDR_ADDRESS, profile->option_base) ||
        !rvswd_memory_write32(transport,
                              RVSWD_FLASH_CTLR_ADDRESS,
                              idle_control | RVSWD_FLASH_CTLR_FAST_PROGRAM | RVSWD_FLASH_CTLR_STRT) ||
        !rvswd_flash_wait_ready(transport, profile, &status, 0x40u, 0x41u)) {
        if (rvswd_flash_last_error_value == 0u) {
...
static bool rvswd_flash_write_option_bytes_halfword(
    ...
    if (!rvswd_flash_wait_ready(transport, profile, &status, 0x51u, 0x52u) ||
        !rvswd_memory_read32(transport, profile, true, RVSWD_FLASH_CTLR_ADDRESS,
                             &control)) {
        if (rvswd_flash_last_error_value == 0u) {
    ...
    if (!rvswd_memory_write32(transport, RVSWD_FLASH_CTLR_ADDRESS,
        ...
        !rvswd_memory_write32(transport,
                              RVSWD_FLASH_CTLR_ADDRESS,
                              idle_control | RVSWD_FLASH_CTLR_OPTER | RVSWD_FLASH_CTLR_STRT) ||
        !rvswd_flash_wait_ready(transport, profile, &status, 0x56u, 0x57u)) {
        if (rvswd_flash_last_error_value == 0u) {
    ...
    if (!rvswd_memory_read32(transport, profile, true, RVSWD_FLASH_CTLR_ADDRESS,
                             &control) ||
        !rvswd_flash_unlock_option_bytes(transport, true) ||
        !rvswd_memory_read32(transport, profile, true, RVSWD_FLASH_CTLR_ADDRESS,
                             &control)) {
    ...
    for (uint32_t index = 0u; index < RVSWD_OPTION_BYTES_WORD_COUNT * 2u; ++index) {
        ...
        if (!rvswd_memory_write32(transport,
            ...
            !rvswd_memory_write16(transport,
                                  profile->option_base + index * 2u, value,
                                  RVSWD_ABSTRACT_TIMEOUT_US) ||
            !rvswd_flash_wait_ready(transport, profile, &status, 0x5cu,
                                    0x5du)) {
            if (rvswd_flash_last_error_value == 0u) {
...
static bool rvswd_flash_unprotect_option_bytes(
    ...
    if (!rvswd_flash_wait_ready(transport, profile, &status, 0x61u, 0x62u) ||
        !rvswd_memory_read32(transport, profile, true, RVSWD_FLASH_CTLR_ADDRESS,
                             &control)) {
        if (rvswd_flash_last_error_value == 0u) {
    ...
    if (!rvswd_memory_write32(transport, RVSWD_FLASH_CTLR_ADDRESS,
        ...
        !rvswd_memory_write32(transport,
                              RVSWD_FLASH_CTLR_ADDRESS,
                              idle_control | RVSWD_FLASH_CTLR_OPTER | RVSWD_FLASH_CTLR_STRT) ||
        !rvswd_flash_wait_ready(transport, profile, &status, 0x66u, 0x67u)) {
        if (rvswd_flash_last_error_value == 0u) {
    ...
    if (!rvswd_memory_write16(transport, profile->option_base,
    ...
    if (!rvswd_flash_wait_ready(transport, profile, &status, 0x6cu, 0x6du)) {
        if (rvswd_flash_last_error_value == 0u) {
...
bool rvswd_flash_set_read_protected(struct rvswd_transport *transport,
                                    const struct rvswd_target_profile *profile,
                                    bool protected) {
    ...
    if (!rvswd_flash_read_protected(transport, profile, &current)) {
        return false;
    }
    ...
    if (!protected && profile->option_write == RVSWD_OPTION_WRITE_FAST_BUFFER) {
        if (!rvswd_flash_unprotect_option_bytes(transport, profile)) {
            return false;
        }
    } else {
        for (uint32_t index = 0u; index < RVSWD_OPTION_BYTES_WORD_COUNT; ++index) {
            if (!rvswd_memory_read32(transport, profile, true,
        ...
        switch (profile->option_write) {
            case RVSWD_OPTION_WRITE_FAST_BUFFER:
                if (!rvswd_flash_write_option_bytes_fast_buffer(
                        transport, profile, option_words)) {
                    return false;
                }
                break;
    ...
    if (!rvswd_reset_and_halt(transport)) {
