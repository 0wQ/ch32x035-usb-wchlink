...
bool rvswd_reset_and_halt(struct rvswd_operation *operation) {
    ...
    if (!rvswd_operation_write_dmi(operation, RVSWD_DMI_CONTROL, 0x80000003u).ok) {
        return false;
    }
...
bool rvswd_soft_reset_and_run(struct rvswd_operation *operation) {
    ...
    if (!rvswd_operation_write_dmi(operation, RVSWD_DMI_CONTROL, 0x00000003u).ok ||
        !rvswd_operation_write_dmi(operation, RVSWD_DMI_CONTROL, 0x00000001u).ok) {
        return false;
    }
    bsp_delay_us(rvswd_reset_release_delay_us);
    return rvswd_operation_write_dmi(operation, RVSWD_DMI_CONTROL, 0x40000001u).ok;
}
...
bool rvswd_reset_and_run(struct rvswd_operation *operation) {
    ...
    bsp_delay_us(rvswd_reset_release_delay_us);
    return rvswd_operation_write_dmi(operation, RVSWD_DMI_CONTROL, 0x00000001u).ok;
}
