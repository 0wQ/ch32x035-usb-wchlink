...
static bool rvswd_gpio_read_memory32_v30x_once(uint32_t address, uint32_t *value) {
    ...
    if (!rvswd_gpio_write_dmi(RVSWD_DMI_DATA1, address)) {
        rvswd_memory_last_error = 0xb1u;
        return false;
    }
    if (!rvswd_gpio_write_dmi(RVSWD_DMI_COMMAND, 0x02200000u)) {
        rvswd_memory_last_error = 0xb2u;
        return false;
    }
    if (!rvswd_gpio_read_dmi(RVSWD_DMI_DATA0, &data)) {
...
static bool rvswd_gpio_write_memory_streaming(uint32_t address,
    ...
    if (!rvswd_gpio_write_dmi(RVSWD_DMI_ABSTRACTAUTO, 0u) ||
        !rvswd_gpio_write_dmi(RVSWD_DMI_ABSTRACTCS, 0x00000700u) ||
        !rvswd_gpio_read_dmi(RVSWD_DMI_HARTINFO, &hartinfo)) {
        return false;
    }
    ...
    if (!rvswd_gpio_write_dmi(RVSWD_DMI_PROGBUF0, 0x41844100u) ||
        !rvswd_gpio_write_dmi(RVSWD_DMI_PROGBUF1, 0x0491c080u) ||
        !rvswd_gpio_write_dmi(RVSWD_DMI_PROGBUF2, 0x9002c184u) ||
        !rvswd_gpio_write_raw_gpr(10u, data0_address) ||
        !rvswd_gpio_write_raw_gpr(11u, data0_address + 4u) ||
        !rvswd_gpio_write_dmi(RVSWD_DMI_DATA1, address)) {
        return false;
    }

    // 第一字通过显式 COMMAND 启动，避免 V30X 在 COMMAND 前开启 autoexec
    if (!rvswd_gpio_write_dmi(
            RVSWD_DMI_DATA0,
            ((uint32_t)data[0u]) |
                ((uint32_t)data[1u] << 8u) |
                ((uint32_t)data[2u] << 16u) |
                ((uint32_t)data[3u] << 24u)) ||
        !rvswd_gpio_write_dmi(RVSWD_DMI_COMMAND,
                               RVSWD_ABSTRACT_COMMAND_EXECUTE) ||
        !rvswd_gpio_wait_abstract_idle(&abstractcs) ||
        ((abstractcs >> 8u) & 0x07u) != 0u ||
        !rvswd_gpio_write_dmi(RVSWD_DMI_ABSTRACTAUTO,
    ...
    for (uint32_t offset = 4u; offset < length; offset += 4u) {
        ...
        if (!rvswd_gpio_write_dmi(RVSWD_DMI_DATA0, value)) {
            (void)rvswd_gpio_write_dmi(RVSWD_DMI_ABSTRACTAUTO, 0u);
            (void)rvswd_gpio_write_dmi(RVSWD_DMI_ABSTRACTCS, 0x00000700u);
            return false;
        }
    }
...
static bool rvswd_gpio_write_memory_slow(uint32_t address, const uint8_t *data,
    ...
    if (!rvswd_gpio_write_dmi(RVSWD_DMI_PROGBUF0, 0x0084a023u)) {
        rvswd_memory_last_error = 0xe2u;
        return false;
    }
    if (!rvswd_gpio_write_dmi(RVSWD_DMI_PROGBUF1, 0x00100073u)) {
...
bool rvswd_gpio_write_memory(uint32_t address, const uint8_t *data, uint32_t length) {
    ...
    if (profile != NULL &&
        ...
        success = rvswd_gpio_write_memory_streaming(address, data, length);
        if (!success) {
...
bool rvswd_gpio_connect(void) {
    ...
    if (rvswd_target_profile_from_family(rvswd_target_family_hint()) == NULL) {
        ...
        rvswd_packet_mode = RVSWD_PACKET_SHORT;
