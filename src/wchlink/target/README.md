...
profile 只保存稳定的目标数据和能力，寄存器访问顺序、前置配置、错误恢复和 loader 执行时序由对应族模块的 C 函数实现。CH32X03X、CH32L103 按官方抓包独立实现，CH32V30X、CH58X、CH59X 的未验证行为以带 TODO 的占位实现接管，不通过公共默认回退隐藏。模块可以绑定无族选择的共享事务 backend，但不能让 backend 反向识别族
