...
extern const uint8_t rvswd_target_ch58x_flash_erase_stub_start[];
extern const uint8_t rvswd_target_ch58x_flash_erase_stub_end[];

static const struct rvswd_target_identity_profile rvswd_target_ch58x_identity = {
...
static const uint32_t rvswd_target_ch58x_debug_data_address = 0xe0000380u;

static const struct rvswd_target_loader_profile rvswd_target_ch58x_loader = {
...
static const struct rvswd_target_profile rvswd_target_ch58x_profile_data = {
    .fast_timing = false,
    .identity = &rvswd_target_ch58x_identity,
    .option = NULL,
    .loader = &rvswd_target_ch58x_loader,
...
static const struct rvswd_target_capabilities rvswd_target_ch58x_capabilities = {
    .packet_mode = RVSWD_PACKET_LONG,
    .chip_info_layout = RVSWD_TARGET_CHIP_INFO_LEGACY,
    .memory_streaming = false,
};

// 使用 CH58X Debug Module 的字节访问程序读取专用身份寄存器
static bool rvswd_target_ch58x_probe_read8(
    ...
    if (value == NULL ||
        !rvswd_debug_write_raw_gpr(operation, 13u,
                                   rvswd_target_ch58x_debug_data_address) ||
        !rvswd_operation_write_dmi(operation, RVSWD_DMI_ABSTRACTCS,
                                   0x00000700u)
             .ok ||
        !rvswd_operation_write_dmi(operation, RVSWD_DMI_PROGBUF0,
                                   0x00058483u)
             .ok ||
...
// 读取 CH58X 专用 8 位 ChipID，并校验族匹配结果
static bool rvswd_target_ch58x_probe_chip_id(
    struct rvswd_operation *operation, uint32_t *chip_id) {
    uint8_t value;

    if (chip_id == NULL ||
        !rvswd_target_ch58x_probe_read8(
            operation, rvswd_target_ch58x_identity.chip_id_address, &value) ||
        !rvswd_target_ch58x_matches_chip_id((uint32_t)value << 24u)) {
        return false;
    }
...
static const struct rvswd_target_probe_ops rvswd_target_ch58x_probe = {
    .read_chip_id = rvswd_target_ch58x_probe_chip_id,
};

// TODO: 当前复制既有 CH5XX loader ABI，待 CH58X 官方抓包冻结执行顺序
...
static const struct rvswd_memory_ops rvswd_target_ch58x_memory = {
    .read32 = rvswd_memory_read32_synchronized,
    .write32 = rvswd_memory_write32_slow,
    .write = rvswd_memory_write_slow,
};

static const struct rvswd_target_loader_ops rvswd_target_ch58x_loader_ops = {
    .prepare = NULL,
    .execute = rvswd_target_ch58x_loader_execute,
};

static const struct rvswd_target_flash_ops rvswd_target_ch58x_flash = {
    .erase_all = rvswd_target_ch58x_flash_erase_all,
...
static const struct rvswd_target_control_ops rvswd_target_ch58x_control = {
    .reset_and_halt = rvswd_reset_and_halt,
    .soft_reset_and_run = rvswd_soft_reset_and_run,
    .reset_and_run = rvswd_reset_and_run,
};

static const struct rvswd_target_module rvswd_target_ch58x = {
    .family = WCHLINK_TARGET_FAMILY_CH58X,
    .matches_chip_id = rvswd_target_ch58x_matches_chip_id,
    .profile = &rvswd_target_ch58x_profile_data,
    .capabilities = &rvswd_target_ch58x_capabilities,
    .probe = &rvswd_target_ch58x_probe,
    .memory = &rvswd_target_ch58x_memory,
    .loader = &rvswd_target_ch58x_loader_ops,
    .flash = &rvswd_target_ch58x_flash,
    .control = &rvswd_target_ch58x_control,
};

// 返回 CH58X 族的完整 module 入口
const struct rvswd_target_module *rvswd_target_ch58x_module(void) {
    return &rvswd_target_ch58x;
}

// 返回 CH58X 族唯一的静态目标描述
const struct rvswd_target_profile *rvswd_target_ch58x_profile(void) {
    return &rvswd_target_ch58x_profile_data;
}

// 使用 CH58X 独立 stub 启动整片擦除流程
bool rvswd_target_ch58x_flash_erase_all(
    struct rvswd_operation *operation,
    const struct rvswd_target_profile *profile) {
    return rvswd_flash_ch5xx_erase_all(
        operation, profile, rvswd_target_ch58x_flash_erase_stub_start,
        rvswd_target_ch58x_flash_erase_stub_end,
        rvswd_target_ch58x_loader_execute);
}

bool rvswd_target_ch58x_matches_chip_id(uint32_t chip_id) {
    uint32_t family = chip_id & 0xff000000u;

    return family == 0x82000000u || family == 0x83000000u;
}
