...
// 保持起始条件中 SWDIO 的高电平窗口
static inline __attribute__((always_inline)) void rvswd_phy_start_half_period(bool fast_timing) {
    (void)fast_timing;
...
// 选择目标回复采样低相位和建立期共用的延时
static inline __attribute__((always_inline)) void rvswd_phy_sample_phase_delay(bool fast_timing) {
    // 采样低相位和上升沿后的建立期使用同一组已验证延时，避免两个名称暗示不存在的时序差异
    if (fast_timing) {
...
// 按指定档位发送一个主机驱动位
static inline __attribute__((always_inline)) void rvswd_phy_drive_bit(bool fast_timing, uint8_t value) {
...
static inline __attribute__((always_inline)) uint8_t rvswd_phy_sample_bit(bool fast_timing) {
    ...
    // 低相位与上升沿后的建立期使用当前档位的已验证等待窗口
    rvswd_phy_clock_low();
    rvswd_phy_sample_phase_delay(fast_timing);
    rvswd_phy_clock_high();
    rvswd_phy_sample_phase_delay(fast_timing);
...
// 将帧中的连续 bit 发送到 SWDIO
void rvswd_phy_gpio_drive_range(bool fast_timing, const uint8_t *frame, uint8_t first_bit, uint8_t bit_count) {
    if (fast_timing) {
        ...
        for (uint8_t bit = 0u; bit < bit_count; ++bit) {
            rvswd_phy_drive_bit(true, (*byte & mask) != 0u ? 1u : 0u);
            mask >>= 1u;
            if (mask == 0u) {
                mask = 0x80u;
                ++byte;
...
// 将目标回复的连续 bit 写回帧缓冲区
void rvswd_phy_gpio_sample_range(bool fast_timing, uint8_t *frame, uint8_t first_bit, uint8_t bit_count) {
    if (fast_timing) {
        ...
        for (uint8_t bit = 0u; bit < bit_count; ++bit) {
            if (rvswd_phy_sample_bit(true) != 0u) {
                *byte |= mask;
            } else {
                *byte &= (uint8_t)~mask;
            }
...
void rvswd_phy_gpio_drive_value(bool fast_timing, uint32_t value, uint8_t bit_count) {
    if (fast_timing) {
        ...
        for (uint8_t bit = bit_count; bit > 0u; --bit) {
            rvswd_phy_drive_bit(true, (uint8_t)((value >> (bit - 1u)) & 1u));
        }
        return;
    }
    for (uint8_t bit = bit_count; bit > 0u; --bit) {
...
// 从 SWDIO 采样指定 bit 数并按高位到低位组合为整数
uint32_t rvswd_phy_gpio_sample_value(bool fast_timing, uint8_t bit_count) {
    ...
    if (fast_timing) {
        ...
        for (uint8_t bit = 0u; bit < bit_count; ++bit) {
            value = (value << 1u) | rvswd_phy_sample_bit(true);
        }
        return value;
    }
    for (uint8_t bit = 0u; bit < bit_count; ++bit) {
...
// 初始化 GPIO，SWCLK 输出高电平，SWDIO 保持输入空闲态
